...
def create_wiki_account(name, password, email):
    ...
    response = opener.open("http://{0}/api.php?action=createaccount".format(g.wiki_url), data_encoded)
    ...
    response2 = opener.open("http://{0}/api.php?action=createaccount".format(g.wiki_url), data_encoded2)
