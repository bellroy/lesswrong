...
create or replace view active as
    select pg_stat_activity.pid, (now() - pg_stat_activity.query_start) as t, pg_stat_activity.query from pg_stat_activity where (pg_stat_activity.query <> '<IDLE>'::text) order by (now() - pg_stat_activity.query_start);
